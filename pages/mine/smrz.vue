	...
	<view>
		<form @submit="formSubmit" >
			...
			<view class="inputArea inputArea2">
				  <input class="uni-input" name="card" type="number"  placeholder="请填写您的身份证号码" placeholder-class="input-placeholder2" />
			</view>
			<view class="tip"><text class="modeIcon3" >&#xe638;</text><text>上传身份证证件</text></view>
			<view class="uploadPicBtn" @click="uploadPic('1')" >
				...
				<view class="picture" :style="{display:!yinyeShow ? 'flex' : 'none'}">
					<image :src="getImgUrl(sfzzmImgUrl)"></image>
				</view>
			</view>
			<view class="uploadPicBtn" @click="uploadPic('2')" :style="{paddingTop:'10rpx'}" >
				...
				<view class="picture" :style="{display:!yinyeShow2 ? 'flex' : 'none'}">
					<image :src="getImgUrl(sfzfmImgUrl)"></image>
				</view>
			</view>
			<!-- 提交 -->
			<view  class="btns">
		...
		methods: {
			...
			formSubmit: function(e) {
			   ...
			   that.$http.requestPost({
				   url:`${that.$baseUrl}/api/user/idCard`,
				   data: newO,
				   success: res=>{ 
					   uni.showToast({
							title: "提交成功", 
